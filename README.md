...
Basically, all aspects of a grid are defined in the Controller using the provided custom builder.
...
Currently there are implementation for 3 datasources ( GORM , list - represents a list of objects stored in the session (or anywhere), custom - fully configurable)
...
Upgrading to 1.1.0
-----------------
 - on installing templates are copied to the /templates/easygrid folder
 ...
 - the labelFormat has a slightly different format ( see the comments )
